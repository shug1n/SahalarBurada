    ...
    public class FormSahaAra : Form
    {
        ...
        private void InitUI()
        {
            this.Controls.Add(UIHelper.HeaderPanelOlustur(
                "📅  Saha Ara", "Tarih ve saat seçerek müsait sahaları listeleyin"));

            var content = new Panel
            {
                Location = new Point(0, 95), Size = new Size(540, 375),
                BackColor = UIHelper.CArkaplan
            };

            // Kullanıcı bilgisi
            string hosGeldi = Oturum.GirisYapildi
                ? $"👤 Hoş geldiniz, {Oturum.AktifKullanici.Ad} {Oturum.AktifKullanici.Soyad}"
                : "👤 Misafir olarak arama yapıyorsunuz";
            content.Controls.Add(UIHelper.LblKucuk(hosGeldi, 50, 20, UIHelper.CMetinAcik));

            // Tarih
            content.Controls.Add(UIHelper.LblAlan("Tarih Seçin:", 50, 55));
            dtpTarih = new DateTimePicker
            {
                Location = new Point(50, 80), Width = 440,
                Font = UIHelper.FNormal,
        ...
        private void BtnListele_Click(object sender, EventArgs e)
        {
            ...
            if (sahalar.Count == 0)
            {
                lblHata.Text = "Seçilen tarih ve saatte müsait saha bulunmamaktadır.";
