...
{
    public class FormSahaOzet : Form
    {
        ...
        private void InitUI()
        {
            this.Controls.Add(UIHelper.HeaderPanelOlustur(
                "📄  Saha Özeti", "Bilgileri kontrol edin; onaylayarak kaydedebilirsiniz"));

            var scroll = new Panel
            {
                Location = new Point(0, 95), Size = new Size(680, 565),
            ...
            // ── Özet kartı ────────────────────────────────────────────
            var kart = UIHelper.KartPanel(x, y, 614, 450);
            kart.Controls.Add(new Label
            {
                Text = "📋 Saha Bilgileri Özeti", Font = UIHelper.FAltBaslik,
                ForeColor = UIHelper.CAna, AutoSize = true, Location = new Point(18, 14)
            });
            ...
            int ky = 52;
            void BilgiSatiri(string etiket, string deger, int yukseklik = 26)
            {
                kart.Controls.Add(new Label
                {
                    Text = etiket, Font = UIHelper.FNormalKalin, ForeColor = UIHelper.CMetin,
                    Location = new Point(20, ky), Size = new Size(145, yukseklik)
                });
                kart.Controls.Add(new Label
                {
                    Text = deger, Font = UIHelper.FNormal, ForeColor = UIHelper.CMetinAcik,
                    Location = new Point(170, ky), AutoSize = false,
                    Size = new Size(430, yukseklik)
                });
                ky += yukseklik + 6;
            }
            ...
            string gunlerStr = string.Join(", ", _saha.MüsaitGunler);
            BilgiSatiri("Müsait Günler:", gunlerStr, 40);

            string saatlerStr = _saha.MüsaitSaatler.Count + " dilim  (" +
                _saha.MüsaitSaatler[0] + " → " +
                _saha.MüsaitSaatler[_saha.MüsaitSaatler.Count - 1] + ")";
            BilgiSatiri("Müsait Saatler:", saatlerStr);

            string aciklama = string.IsNullOrWhiteSpace(_saha.Aciklama) ? "—" : _saha.Aciklama;
            BilgiSatiri("Açıklama:", aciklama, 48);
            ...
            btnDuzenle.Click += (s, e) => this.Close(); // FormSahaEkle geri açılır
